...
export default function BlazeryPage() {
  ...
  const petState = useMemo(() => {
    ...
    else if (blazeResult === "success") state = "excited";
    else if (happiness > 70) state = "happy";
    
    return { state, happiness, health };
  }, [minerState, isWriting, isConfirming, blazeResult]);
      ...
      >
        <div className="flex flex-1 flex-col overflow-y-auto space-y-2">
          ...
          <PetStats
            happiness={petState.happiness}
            health={petState.health}
            energy={minerState ? formatTokenAmount(minerState.glazed, DONUT_DECIMALS, 2) : "0"}
            age={minerState ? formatGlazeTime(glazeElapsedSeconds) : "0s"}
          />
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-cyan-300 border-4 border-black rounded-xl p-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="grid grid-cols-2 gap-2">
            {profitLoss && (
              <div className={`border-4 border-black rounded-xl p-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-center ${
                profitLoss.isProfitable ? "bg-green-300" : "bg-red-300"
              }`}>
                <div className="text-[10px] font-black text-black text-center leading-tight">
                  {profitLoss.isProfitable ? "💰 GAIN" : "📉 LOSS"}
                </div>
                <div className="text-[9px] font-bold text-black/80 text-center mt-0.5">
                  {profitLoss.profitLoss >= 0 ? "+" : ""}${Math.abs(profitLoss.profitLoss) > 999 ? (profitLoss.profitLoss / 1000).toFixed(1) + "k" : profitLoss.profitLoss.toFixed(0)}
                </div>
              </div>
            )}

            <Button
              className="h-14 rounded-2xl bg-gradient-to-b from-orange-400 to-orange-600 border-4 border-black text-black text-lg font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
              onClick={handleBlaze}
          ...
          <div className="bg-white border-4 border-black rounded-xl p-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center justify-between gap-2">
              ...
              <a
                ...
                className="text-[10px] font-black text-pink-600 hover:text-pink-700 transition-colors"
              >
                GET LP →
              </a>
            </div>
          ...
          <div className="bg-lime-300 border-4 border-black rounded-xl p-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div className="text-[10px] font-black text-black/80 space-y-1">
